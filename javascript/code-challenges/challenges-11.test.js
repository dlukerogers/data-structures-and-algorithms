...
const count = (target, input) => {
  // Solution code here...
  let total = input
    .map(innerArr => innerArr.filter(num => num === target).length)
    .reduce((accumulator, occurences) => accumulator + occurences, 0);
  return total;
};
...
const totalSum = (input) => {
  // Solution code here...
  let totalSum = input
    .map(innerArr => innerArr.reduce((accumulator, currentNum) => currentNum + accumulator, 0))
    .reduce((accumulator, currentNum) => currentNum + accumulator, 0);
  return totalSum
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let result = input
    .map(innerArr => innerArr.filter(num => num % 5 === 0 && typeof num == 'number')) // [[0,5], [10], []]
    .map(innerArr => innerArr.map(num => 2 ** num));
  return result;
};
...
let findShortest = (data) => {
  // Solution code here...
  let shortestChar = data
    .reduce((shortest, currentChar) => Number(shortest.height) < Number(currentChar.height) ? shortest : currentChar).name;
  return shortestChar;
};
