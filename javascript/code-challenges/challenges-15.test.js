...
const screenForNames = (arr) => {
  // Solution code here...
  let regex = /^(Mr\.|Mrs\.|Ms\.|Dr\.)\s?[a-z|A-Z]+$/;
  let newArr = [];
...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map(str => {
    let startLetters = str.substring(0,1).toUpperCase();
    let leftOverStr = str.substring(1);
    return startLetters + leftOverStr;
  })
};
...
let biggerThanLuke = (arr) => {
  ...
  let charsBiggerThanLuke = [];
  for (let i = 0; i < arr.length; i++) {
    if (parseInt(arr[i].mass) > lukeWeight) {
      charsBiggerThanLuke.push(arr[i].name);
    }
  }
  return charsBiggerThanLuke.join(' - ');
};
...
const isSecure = (url) => {
  ...
  if (url.startsWith('https://')) {
    let index = url.indexOf('https:') + 'https:'.length;
    let nextCharacter = url.charAt(index);
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  const check = (board, row1, column1, row2, column2, row3, column3) => {
    const value1 = board[row1][column1];
    const value2 = board[row2][column2];
    const value3 = board[row3][column3];
    return value1 !== '' && value1 === value2 && value2 === value3;
  };
  for (let i = 0; i < 3; i++) {
    if (check(board, i, 0, i, 1, i, 2)) {
      return true;
    }
  }

  for (let j = 0; j < 3; j++) {
    if (check(board, 0, j, 1, j, 2, j)) {
      return true;
    }
  }

  if (check(board, 0, 0, 1, 1, 2, 2) || check(board, 0, 2, 1, 1, 2, 0)) {
    return true;
  }
